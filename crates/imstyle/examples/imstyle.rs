...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut imgui = ImGuiSdl3::new(&device, &window, |ctx| {
        ...
        let toml_str = fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/examples/cherry.toml"))
            .expect("failed to read theme file");
        let imguistyle = ImStyle::from_toml(&toml_str).expect("failed to parse theme file");
        imguistyle.set_current(ctx);
